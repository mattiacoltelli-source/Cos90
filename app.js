(() => {
  const CORE = window.CINE_CORE || {};

  const API_KEY = CORE.API_KEY || "f8d5e378edf5128176f0d89f49310151";
  const BASE_URL = CORE.BASE_URL || "https://api.themoviedb.org/3";
  const IMG = CORE.IMG || "https://image.tmdb.org/t/p/w500";
  const TONIGHT_AUTO_COOLDOWN_MS = CORE.TONIGHT_AUTO_COOLDOWN_MS || 20000;
  const SUGGEST_HISTORY_KEY = CORE.SUGGEST_HISTORY_KEY || "cineTrackerSuggestHistory";
  const SUGGEST_HISTORY_MAX = CORE.SUGGEST_HISTORY_MAX || 40;

  const GENRE_MAP = CORE.GENRE_MAP || {
    28:"Azione",
    12:"Avventura",
    16:"Animazione",
    35:"Commedia",
  ...
  const GENRE_NAME_TO_ID = CORE.GENRE_NAME_TO_ID || {
    "Azione": 28,
    "Avventura": 12,
    "Animazione": 16,
    "Commedia": 35,
  ...
  function normalizedItem(item) {
    return {
      ...
      overview: item.overview ? (item.overview.length > 320 ? item.overview.slice(0, 320) + "..." : item.overview) : "",
  ...
  function renderTop100() {
    ...
    if (!rest.length) {
      els.top100List.innerHTML = `<div class="insightItem">Per ora hai solo il podio. Continua a votare per riempire tutta la classifica.</div>`;
      return;
    }

    els.top100List.innerHTML = rest.map((item, index) => `
  ...
  function renderTop100Series() {
    ...
    if (!rest.length) {
      els.top100SeriesList.innerHTML = `<div class="insightItem">Per ora hai solo il podio. Continua a votare per riempire tutta la classifica.</div>`;
      return;
    }

    els.top100SeriesList.innerHTML = rest.map((item, index) => `
      <div class="rankingRow">
        <div class="rankingPos">${index + 4}</div>
        <div class="rankingPoster" style="background-image:url('${posterUrl(item.poster_path)}')"></div>
        <div class="rankingInfo">
  ...
  function renderLibraryList() {
    ...
    if (currentLibraryMode === "watch") {
      els.libraryTitle.textContent = currentLibraryGenre === "all"
        ? "Watchlist"
    ...
    if (!items.length) {
      ...
      els.libraryEmpty.textContent = currentLibraryMode === "watch"
        ? (currentLibraryGenre === "all"
      ...
      <div class="listItem">
        ...
        <div>
          ...
          <div class="chips">
            ...
            ${item.comment ? `<div class="chip">${escapeHtml(item.comment).slice(0, 36)}${item.comment.length > 36 ? "..." : ""}</div>` : ""}
          </div>
          <div class="actions">
  ...
  function renderHomeShelves() {
    ...
    els.openWatchAll.classList.toggle("hidden", db.watchlist.length === 0);
    els.openSeenMovies.classList.toggle("hidden", db.seen.filter(x => x.media_type === "movie").length === 0);
    els.openSeenSeries.classList.toggle("hidden", db.seen.filter(x => x.media_type === "tv").length === 0);
  ...
  async function discoverByTaste() {
    ...
    try {
      ...
      const poster = chosen.poster_path
        ? `<img src="${IMG}${chosen.poster_path}" alt="${escapeHtml(chosen.title)}" style="width:74px;border-radius:12px;float:right;margin-left:12px;border:1px solid rgba(255,255,255,.08);">`
        : "";
  ...
  window.addEventListener("popstate", (e) => {
    ...
    if (els.screens[name]) {
      switchScreen(name, { skipHistory: true });
    }
  });
  ...
  function setupLibraryFilters() {
    els.libraryFilters.forEach(btn => {
      btn.addEventListener("click", () => {
        setLibraryFilter(btn.dataset.filter);
      });
    });
  }

  function setupSearch() {
    els.searchBtn.addEventListener("click", searchTitles);
  ...
  function setupNav() {
    ...
    els.libraryBackBtn.addEventListener("click", () => switchScreen("home"));
    els.detailBackBtn.addEventListener("click", () => switchScreen(previousScreen || "home"));

    els.openWatchAll.addEventListener("click", () => openLibrary("watch", "all"));
    els.openSeenMovies.addEventListener("click", () => openLibrary("seen", "movie"));
    els.openSeenSeries.addEventListener("click", () => openLibrary("seen", "series"));
  }

  function setupTonightButtons() {
    els.recommendBtn.addEventListener("click", () => recommendTonightFive({ auto: false }));
    els.discoverBtn.addEventListener("click", discoverByTaste);
    els.classicBtn.addEventListener("click", suggestClassic);
  }

  function setupBackup() {
    els.exportBtn2.addEventListener("click", exportBackup);
  ...
  function setupDetailButtons() {
    ...
    els.detailSaveNoteBtn.addEventListener("click", saveDetailNotes);
  ...
  function setupDelegatedClicks() {
    document.addEventListener("click", async function(e) {
      ...
      try {
        ...
        if (seenBtn) {
          await addSeen(seenBtn.dataset.type, seenBtn.dataset.id);
          return;
        }

        if (watchBtn) {
          await addWatch(watchBtn.dataset.type, watchBtn.dataset.id);
          return;
        }

        if (detailsBtn) {
          await showDetails(detailsBtn.dataset.type, detailsBtn.dataset.id);
          return;
        }

        if (removeSeenBtn) {
          removeSeen(removeSeenBtn.dataset.key);
          return;
        }

        if (removeWatchBtn) {
          removeWatch(removeWatchBtn.dataset.key);
          return;
        }

        if (moveWatchBtn) {
          moveWatchToSeen(moveWatchBtn.dataset.key);
          return;
        }
