...
function renderSearchResults(items, db) {
  return items.map(item => {
    const seen = !!db.seen.find(x => uniqueKey(x) === uniqueKey(item));
    const watch = !!db.watchlist.find(x => uniqueKey(x) === uniqueKey(item));
      ...
      <div class="poster-card">
        <div class="poster-card__img" style="background-image:url('${posterUrl(item.poster_path)}')">
          ...
          <div class="poster-card__overlay">
            <button class="btn--icon action-seen" data-id="${item.id}" data-type="${item.media_type}">
              ${seen ? "✓ Visto" : "+ Visto"}
            </button>
            <button class="btn--icon action-watch" data-id="${item.id}" data-type="${item.media_type}">
              ${watch ? "★ Lista" : "♡ Lista"}
            </button>
          </div>
        </div>
        <div class="poster-card__info">
          ...
          <div class="poster-card__meta">${escapeHtml(item.year)} · ${mediaLabel(item)}</div>
          <button class="btn btn--ghost btn--sm action-details" data-id="${item.id}" data-type="${item.media_type}">
            Scheda →
          </button>
        </div>
      </div>
  ...
  return `
    <div class="discover-result">
      ...
      <div class="discover-result__body">
        ...
        <div class="discover-result__meta">${escapeHtml(chosen.year)} · ${mediaLabel(chosen)} · ★ ${escapeHtml(rating)}/10</div>
